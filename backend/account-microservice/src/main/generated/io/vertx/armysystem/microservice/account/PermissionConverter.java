...
public class PermissionConverter {
  ...
  public static void fromJson(JsonObject json, Permission obj) {
    if (json.getValue("actions") instanceof JsonArray) {
      java.util.ArrayList<io.vertx.armysystem.microservice.account.Action> list = new java.util.ArrayList<>();
      json.getJsonArray("actions").forEach( item -> {
        if (item instanceof String)
          list.add(io.vertx.armysystem.microservice.account.Action.valueOf((String)item));
      });
      obj.setActions(list);
    }
