...
import io.vertx.armysystem.business.common.standard.PlaceRequirement;
...
public class PlaceRequirementServiceImpl extends MongoRepositoryWrapper implements ServiceBase, CRUDService {
  private static final Logger logger = LoggerFactory.getLogger(PlaceRequirementServiceImpl.class);
  private final Vertx vertx;

  public PlaceRequirementServiceImpl(Vertx vertx, JsonObject config) {
    super(vertx, config);
  ...
  public CRUDService addOne(JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    validateParams(item, true)
        .compose(o -> this.insertOne(getCollectionName(), new PlaceRequirement(o).toJson()))
        .setHandler(resultHandler);
  ...
  public CRUDService updateOne(String id, JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    this.update(getCollectionName(), getIdQuery(id), new PlaceRequirement(item).toJson())
        .setHandler(ar -> {
          if (ar.succeeded()) {
  ...
  private Future<JsonObject> validateParams(JsonObject item, Boolean forAdd) {
    ...
    PlaceRequirement requirement = new PlaceRequirement(item);
