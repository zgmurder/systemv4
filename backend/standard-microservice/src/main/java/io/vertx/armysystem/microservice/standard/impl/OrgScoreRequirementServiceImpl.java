...
import io.vertx.armysystem.business.common.standard.OrgScoreRequirement;
...
public class OrgScoreRequirementServiceImpl extends MongoRepositoryWrapper implements ServiceBase, CRUDService {
  private static final Logger logger = LoggerFactory.getLogger(OrgScoreRequirementServiceImpl.class);
  private final Vertx vertx;

  public OrgScoreRequirementServiceImpl(Vertx vertx, JsonObject config) {
    super(vertx, config);
  ...
  public String getServiceName() {
    return "standard-OrgScoreRequirement-eb-service";
  }

  @Override
  public String getServiceAddress() {
    return "service.standard.OrgScoreRequirement";
  }
  ...
  public CRUDService addOne(JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    validateParams(item, true)
        .compose(o -> this.insertOne(getCollectionName(), new OrgScoreRequirement(o).toJson()))
        .setHandler(resultHandler);
  ...
  public CRUDService updateOne(String id, JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    this.update(getCollectionName(), getIdQuery(id), new OrgScoreRequirement(item).toJson())
        .setHandler(ar -> {
          if (ar.succeeded()) {
  ...
  private Future<JsonObject> validateParams(JsonObject item, Boolean forAdd) {
    ...
    OrgScoreRequirement requirement = new OrgScoreRequirement(item);
