...
import io.vertx.armysystem.business.common.standard.TimeRequirement;
...
public class TimeRequirementServiceImpl extends MongoRepositoryWrapper implements ServiceBase, CRUDService {
  private static final Logger logger = LoggerFactory.getLogger(TimeRequirementServiceImpl.class);
  private final Vertx vertx;

  public TimeRequirementServiceImpl(Vertx vertx, JsonObject config) {
    super(vertx, config);
  ...
  public String getServiceName() {
    return "standard-TimeRequirement-eb-service";
  }

  @Override
  public String getServiceAddress() {
    return "service.standard.TimeRequirement";
  }
  ...
  public CRUDService addOne(JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    validateParams(item, true)
        .compose(o -> this.insertOne(getCollectionName(), new TimeRequirement(o).toJson()))
        .setHandler(resultHandler);
  ...
  public CRUDService updateOne(String id, JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    this.update(getCollectionName(), getIdQuery(id), new TimeRequirement(item).toJson())
        .setHandler(ar -> {
          if (ar.succeeded()) {
  ...
  private Future<JsonObject> validateParams(JsonObject item, Boolean forAdd) {
    ...
    TimeRequirement requirement = new TimeRequirement(item);
