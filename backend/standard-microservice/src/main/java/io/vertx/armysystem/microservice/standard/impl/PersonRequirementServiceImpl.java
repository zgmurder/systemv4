...
import io.vertx.armysystem.business.common.standard.PersonRequirement;
...
public class PersonRequirementServiceImpl extends MongoRepositoryWrapper implements ServiceBase, CRUDService {
  private static final Logger logger = LoggerFactory.getLogger(PersonRequirementServiceImpl.class);
  private final Vertx vertx;

  public PersonRequirementServiceImpl(Vertx vertx, JsonObject config) {
    super(vertx, config);
  ...
  public CRUDService addOne(JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    validateParams(item, true)
        .compose(o -> this.insertOne(getCollectionName(), new PersonRequirement(o).toJson()))
        .setHandler(resultHandler);
  ...
  public CRUDService updateOne(String id, JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    this.update(getCollectionName(), getIdQuery(id), new PersonRequirement(item).toJson())
        .setHandler(ar -> {
          if (ar.succeeded()) {
  ...
  private Future<JsonObject> validateParams(JsonObject item, Boolean forAdd) {
    ...
    PersonRequirement requirement = new PersonRequirement(item);
