...
import io.vertx.armysystem.business.common.dictionary.TrainStep;
...
public class TrainStepServiceImpl extends MongoRepositoryWrapper implements CRUDService, ServiceBase {
  private final Vertx vertx;

  public TrainStepServiceImpl(Vertx vertx, JsonObject config) {
    super(vertx, config);
  ...
  public String getServiceName() {
    return "dictionary-TrainStep-eb-service";
  }

  @Override
  public String getServiceAddress() {
    return "service.dictionary.TrainStep";
  }
  ...
  public CRUDService addOne(JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    this.insertOne(getCollectionName(), new TrainStep(item).toJson())
        .setHandler(resultHandler);
  ...
  public CRUDService updateOne(String id, JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    this.update(getCollectionName(), getIdQuery(id), new TrainStep(item).toJson())
        .setHandler(ar -> {
          if (ar.succeeded()) {
