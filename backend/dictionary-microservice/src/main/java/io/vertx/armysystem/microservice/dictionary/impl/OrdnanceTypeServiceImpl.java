...
import io.vertx.armysystem.business.common.dictionary.OrdnanceType;
...
public class OrdnanceTypeServiceImpl extends MongoRepositoryWrapper implements CRUDService, ServiceBase {
  private final Vertx vertx;

  public OrdnanceTypeServiceImpl(Vertx vertx, JsonObject config) {
    super(vertx, config);
  ...
  public String getServiceName() {
    return "dictionary-OrdnanceType-eb-service";
  }

  @Override
  public String getServiceAddress() {
    return "service.dictionary.OrdnanceType";
  }
  ...
  public CRUDService addOne(JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    this.insertOne(getCollectionName(), new OrdnanceType(item).toJson())
        .setHandler(resultHandler);
  ...
  public CRUDService updateOne(String id, JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    this.update(getCollectionName(), getIdQuery(id), new OrdnanceType(item).toJson())
        .setHandler(ar -> {
          if (ar.succeeded()) {
