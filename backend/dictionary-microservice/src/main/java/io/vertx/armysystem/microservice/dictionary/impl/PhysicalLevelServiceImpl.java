...
import io.vertx.armysystem.business.common.dictionary.PhysicalLevel;
...
public class PhysicalLevelServiceImpl extends MongoRepositoryWrapper implements CRUDService, ServiceBase {
  private final Vertx vertx;

  public PhysicalLevelServiceImpl(Vertx vertx, JsonObject config) {
    super(vertx, config);
  ...
  public String getServiceName() {
    return "dictionary-PhysicalLevel-eb-service";
  }

  @Override
  public String getServiceAddress() {
    return "service.dictionary.PhysicalLevel";
  }
  ...
  public CRUDService addOne(JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    this.insertOne(getCollectionName(), new PhysicalLevel(item).toJson())
        .setHandler(resultHandler);
  ...
  public CRUDService updateOne(String id, JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    this.update(getCollectionName(), getIdQuery(id), new PhysicalLevel(item).toJson())
        .setHandler(ar -> {
          if (ar.succeeded()) {
