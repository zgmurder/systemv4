...
import io.vertx.armysystem.business.common.dictionary.SportCategory;
...
public class SportCategoryServiceImpl extends MongoRepositoryWrapper implements CRUDService, ServiceBase {
  private final Vertx vertx;

  public SportCategoryServiceImpl(Vertx vertx, JsonObject config) {
    super(vertx, config);
  ...
  public String getServiceName() {
    return "dictionary-SportCategory-eb-service";
  }

  @Override
  public String getServiceAddress() {
    return "service.dictionary.SportCategory";
  }
  ...
  public CRUDService addOne(JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    this.insertOne(getCollectionName(), new SportCategory(item).toJson())
        .setHandler(resultHandler);
  ...
  public CRUDService updateOne(String id, JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    this.update(getCollectionName(), getIdQuery(id), new SportCategory(item).toJson())
        .setHandler(ar -> {
          if (ar.succeeded()) {
