...
import io.vertx.armysystem.business.common.dictionary.TrainerLevel;
...
public class TrainerLevelServiceImpl extends MongoRepositoryWrapper implements CRUDService, ServiceBase {
  private final Vertx vertx;

  public TrainerLevelServiceImpl(Vertx vertx, JsonObject config) {
    super(vertx, config);
  ...
  public String getServiceName() {
    return "dictionary-TrainerLevel-eb-service";
  }

  @Override
  public String getServiceAddress() {
    return "service.dictionary.TrainerLevel";
  }
  ...
  public CRUDService addOne(JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    this.insertOne(getCollectionName(), new TrainerLevel(item).toJson())
        .setHandler(resultHandler);
  ...
  public CRUDService updateOne(String id, JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    this.update(getCollectionName(), getIdQuery(id), new TrainerLevel(item).toJson())
        .setHandler(ar -> {
          if (ar.succeeded()) {
