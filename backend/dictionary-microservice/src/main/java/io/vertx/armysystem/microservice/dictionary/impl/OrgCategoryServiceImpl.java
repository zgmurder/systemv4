...
import io.vertx.armysystem.business.common.dictionary.OrgCategory;
...
public class OrgCategoryServiceImpl extends MongoRepositoryWrapper implements CRUDService, ServiceBase {
  private final Vertx vertx;

  public OrgCategoryServiceImpl(Vertx vertx, JsonObject config) {
    super(vertx, config);
  ...
  public String getServiceName() {
    return "dictionary-OrgCategory-eb-service";
  }

  @Override
  public String getServiceAddress() {
    return "service.dictionary.OrgCategory";
  }
  ...
  public CRUDService addOne(JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    this.insertOne(getCollectionName(), new OrgCategory(item).toJson())
        .setHandler(resultHandler);
  ...
  public CRUDService updateOne(String id, JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    this.update(getCollectionName(), getIdQuery(id), new OrgCategory(item).toJson())
        .setHandler(ar -> {
          if (ar.succeeded()) {
