...
import io.vertx.armysystem.business.common.dictionary.WeatherType;
...
public class WeatherTypeServiceImpl extends MongoRepositoryWrapper implements CRUDService, ServiceBase {
  private final Vertx vertx;

  public WeatherTypeServiceImpl(Vertx vertx, JsonObject config) {
    super(vertx, config);
  ...
  public String getServiceName() {
    return "dictionary-WeatherType-eb-service";
  }

  @Override
  public String getServiceAddress() {
    return "service.dictionary.WeatherType";
  }
  ...
  public CRUDService addOne(JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    this.insertOne(getCollectionName(), new WeatherType(item).toJson())
        .setHandler(resultHandler);
  ...
  public CRUDService updateOne(String id, JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    this.update(getCollectionName(), getIdQuery(id), new WeatherType(item).toJson())
        .setHandler(ar -> {
          if (ar.succeeded()) {
