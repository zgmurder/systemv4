...
import io.vertx.armysystem.business.common.dictionary.OrgProperty;
...
public class OrgPropertyServiceImpl extends MongoRepositoryWrapper implements CRUDService, ServiceBase {
  private final Vertx vertx;

  public OrgPropertyServiceImpl(Vertx vertx, JsonObject config) {
    super(vertx, config);
  ...
  public String getServiceName() {
    return "dictionary-OrgProperty-eb-service";
  }

  @Override
  public String getServiceAddress() {
    return "service.dictionary.OrgProperty";
  }
  ...
  public CRUDService addOne(JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    this.insertOne(getCollectionName(), new OrgProperty(item).toJson())
        .setHandler(resultHandler);
  ...
  public CRUDService updateOne(String id, JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    this.update(getCollectionName(), getIdQuery(id), new OrgProperty(item).toJson())
        .setHandler(ar -> {
          if (ar.succeeded()) {
