...
import io.vertx.armysystem.business.common.dictionary.GunnerType;
...
public class GunnerTypeServiceImpl extends MongoRepositoryWrapper implements CRUDService, ServiceBase {
  private final Vertx vertx;

  public GunnerTypeServiceImpl(Vertx vertx, JsonObject config) {
    super(vertx, config);
  ...
  public String getServiceName() {
    return "dictionary-GunnerType-eb-service";
  }

  @Override
  public String getServiceAddress() {
    return "service.dictionary.GunnerType";
  }
  ...
  public CRUDService addOne(JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    this.insertOne(getCollectionName(), new GunnerType(item).toJson())
        .setHandler(resultHandler);
  ...
  public CRUDService updateOne(String id, JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    this.update(getCollectionName(), getIdQuery(id), new GunnerType(item).toJson())
        .setHandler(ar -> {
          if (ar.succeeded()) {
