...
import io.vertx.armysystem.business.common.dictionary.MilitaryRank;
...
public class MilitaryRankServiceImpl extends MongoRepositoryWrapper implements CRUDService, ServiceBase {
  private final Vertx vertx;

  public MilitaryRankServiceImpl(Vertx vertx, JsonObject config) {
    super(vertx, config);
  ...
  public String getServiceName() {
    return "dictionary-MilitaryRank-eb-service";
  }

  @Override
  public String getServiceAddress() {
    return "service.dictionary.MilitaryRank";
  }
  ...
  public CRUDService addOne(JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    this.insertOne(getCollectionName(), new MilitaryRank(item).toJson())
        .setHandler(resultHandler);
  ...
  public CRUDService updateOne(String id, JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    this.update(getCollectionName(), getIdQuery(id), new MilitaryRank(item).toJson())
        .setHandler(ar -> {
          if (ar.succeeded()) {
