...
import io.vertx.armysystem.business.common.dictionary.SupporterMajor;
...
public class SupporterMajorServiceImpl extends MongoRepositoryWrapper implements CRUDService, ServiceBase {
  private final Vertx vertx;

  public SupporterMajorServiceImpl(Vertx vertx, JsonObject config) {
    super(vertx, config);
  ...
  public String getServiceName() {
    return "dictionary-SupporterMajor-eb-service";
  }

  @Override
  public String getServiceAddress() {
    return "service.dictionary.SupporterMajor";
  }
  ...
  public CRUDService addOne(JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    this.insertOne(getCollectionName(), new SupporterMajor(item).toJson())
        .setHandler(resultHandler);
  ...
  public CRUDService updateOne(String id, JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    this.update(getCollectionName(), getIdQuery(id), new SupporterMajor(item).toJson())
        .setHandler(ar -> {
          if (ar.succeeded()) {
