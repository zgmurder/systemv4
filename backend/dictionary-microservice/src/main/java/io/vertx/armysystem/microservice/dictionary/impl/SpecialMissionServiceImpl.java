...
import io.vertx.armysystem.business.common.dictionary.SpecialMission;
...
public class SpecialMissionServiceImpl extends MongoRepositoryWrapper implements CRUDService, ServiceBase {
  private final Vertx vertx;

  public SpecialMissionServiceImpl(Vertx vertx, JsonObject config) {
    super(vertx, config);
  ...
  public String getServiceName() {
    return "dictionary-SpecialMission-eb-service";
  }

  @Override
  public String getServiceAddress() {
    return "service.dictionary.SpecialMission";
  }
  ...
  public CRUDService addOne(JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    this.insertOne(getCollectionName(), new SpecialMission(item).toJson())
        .setHandler(resultHandler);
  ...
  public CRUDService updateOne(String id, JsonObject item, JsonObject principal, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    this.update(getCollectionName(), getIdQuery(id), new SpecialMission(item).toJson())
        .setHandler(ar -> {
          if (ar.succeeded()) {
