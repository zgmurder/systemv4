...
export const handleExportCallBack=(sheet, excelName, tableData)=> {
    ...
    let rangeArr = [
        ...
        { range: 'E7:L7', value: '训练弹药', border: true, bold: true},
        ...
        { range: 'E8:H8', value: '首长机关月消耗量(人均/总数)', border: true},
        ...
        { range: 'G10:I10', value: '月消耗量', border: true},
        { range: 'J10:L10', value: '累计消耗量', border: true},
        { range: 'A11:B11', value: '区分', border: true},
        { range: 'G11:H11', value: '本级训练（人均/总数）', border: true},
        { range: 'J11:K11', value: '本级训练（人均/总数）', border: true},
    ...
    let cellArr = [
        ...
        { cell: 'C11', value: '本月人均完成量', border: true},
        { cell: 'D11', value: '累计人均完成量', border: true},
        { cell: 'I11', value: '其他', border: true},
        { cell: 'L11', value: '其他', border: true},
    ];
    for(let i = 0; i< bulletStatLength(tableData); i++){
        ...
        sheet.range(`G${12+i}:H${12+i}`).value(((tableData.bulletStat[i]||{}).trainAverage && (tableData.bulletStat[i]||{}).train) && `${(tableData.bulletStat[i]||{}).trainAverage}/${(tableData.bulletStat[i]||{}).train}` || '').merged(true).style({ border: true,horizontalAlignment: 'center',verticalAlignment: 'center',});
        sheet.cell(`I${12+i}`).value((tableData.bulletStat[i]||{}).others || '').style({border: true,horizontalAlignment: 'center',verticalAlignment: 'center',});
        sheet.range(`J${12+i}:K${12+i}`).value(((tableData.bulletStat[i]||{}).totalTrainAverage && (tableData.bulletStat[i]||{}).totalTrain) && `${(tableData.bulletStat[i]||{}).totalTrainAverage}/${(tableData.bulletStat[i]||{}).totalTrain}` || '').merged(true).style({  border: true,horizontalAlignment: 'center',verticalAlignment: 'center',});
        sheet.cell(`L${12+i}`).value((tableData.bulletStat[i]||{}).totalOthers || '').style({ border: true,horizontalAlignment: 'center',verticalAlignment: 'center',});
    }
    let rowArr = [
        {row: 1, height: 25},
        {row: 2, height: 5},
